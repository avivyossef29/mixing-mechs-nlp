...
# 1. הגדרת נתיבים ב-Volume (כאן יש 23TB פנויים!)
...
# פתרון קריטי ל-Quota: העברת כל תיקיות ה-Cache ל-Volume
...
echo "=== Step 2: Build Tools & Visualization ==="
# התקנה ישירה כדי שיהיו זמינים לקימפול
...
echo "=== Step 3: Compiling Mamba Kernels (The Hard Part) ==="
export CUDA_HOME=/usr/local/cuda-12.2
export PATH=$CUDA_HOME/bin:$PATH

# התקנה עם דגל שמכריח שימוש בחבילות שכבר התקנו (בלי בידוד)
# ושימוש ב-Cache שב-Volume בלבד
pip install --no-cache-dir --no-build-isolation causal-conv1d==1.6.0 mamba-ssm==2.1.0

echo "=== Step 4: Final YAML Sync (Skip problematic packages) ==="
# אנחנו מעדכנים מה-YAML אבל מוודאים שלא ינסה להתקין שוב את הקרנלים
conda env update -p "$MY_ENV_PATH" -f "mamba2-gpu_environment.yml"

python -c "import torch; import mamba_ssm; print('SUCCESS: Mamba2 is ready!')"
